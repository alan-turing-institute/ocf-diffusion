...
#SBATCH --job-name DiffusionModel-validation
...
if [ ! -d "ocf-diffusion" ]; then
    echo "Repo does not exist; cloning..."
    git clone https://github.com/alan-turing-institute/ocf-diffusion.git
fi
cd ocf-diffusion
...
# run validation
cloudcasting validate --model-file="models/model.py"
